...
import { resetInput } from '../core/input.ts';
import './inspector.css';
import type { Scene } from '@babylonjs/core/scene';
import type { InspectorToken } from '@babylonjs/inspector';

/** Inspector code and its UI dependencies are fetched only on the first Ctrl+I. */
export function installInspectorShortcut(scene: Scene): void {
    let token: InspectorToken | null = null;
    let modulePromise: Promise<typeof import('@babylonjs/inspector')> | null = null;
    ...
    const toggle = async (): Promise<void> => {
        ...
        try {
            ...
            token = ShowInspector(scene, {
                ...
                sidePaneRemapper: (pane: { key: string }) => ({
