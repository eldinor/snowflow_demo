...
import {
    S, SCHEMA, set, applyPreset,
    type SettingGroup, type SettingKey, type SettingValue, type PresetName,
} from "../core/settings.ts";
import { stats, systemMs, FrameGraph, spikes, resetSpikes } from "../core/perf.ts";
...
type TextElement = HTMLElement & { _v?: string };
...
export class Overlay {
    ...
    readonly readouts: Record<string, TextElement> = {};
    readonly budgetEl: HTMLDivElement;
    readonly budgetRows = new Map<string, TextElement>();
    rig: CameraRig | null;
    character: CharacterController | null;
    readonly poseEl: HTMLDivElement;
    readonly presetBtns: Partial<Record<PresetName, HTMLButtonElement>> = {};
    readonly widgets: OverlayWidget[] = [];
    private readonly _lastText: Record<string, string>;
    private _acc = 0;
    private _graphAcc = 0;
    private _camAcc = 0;
    private _pose = "";

    /** Live systems are optional so the overlay can be constructed before `attach()`. */
    constructor(refs: OverlayRefs = {}) {
        ...
        el.appendChild(pr);
        for (const name of ["ultra", "high", "balanced"] as const satisfies readonly PresetName[]) {
     ...
     */
    attach(refs: OverlayRefs): void {
        if (refs.rig) this.rig = refs.rig;
        if (refs.character) this.character = refs.character;
    }

    private _mkNum(parent: HTMLElement, key: string, label: string): void {
    ...
    private _mkGroup(group: SettingGroup): void {
        ...
        for (let i = 0; i < group.items.length; i++) {
            ...
            if (it.t === "f") {
                ...
                v.className = "val";
                v.textContent = fmtNum(Number(S[it.k]), it.step ?? 1);
                r.oninput = () => {
                    const n = parseFloat(r.value);
                    setDynamic(it.k, n);
                    v.textContent = fmtNum(n, it.step ?? 1);
                };
                row.appendChild(r);
                row.appendChild(v);
                this.widgets.push({
                ...
                const options = it.opts ?? [];
                for (let o = 0; o < options.length; o++) {
                    const op = document.createElement("option");
                    op.value = options[o];
                    op.textContent = options[o];
                    sel.appendChild(op);
                }
                sel.value = String(S[it.k]);
                sel.onchange = () => setDynamic(it.k, sel.value);
                row.appendChild(sel);
    ...
    private _syncPresets(): void {
        for (const k in this.presetBtns) {
            this.presetBtns[k as PresetName]?.classList.toggle("on", S.preset === k);
        }
    }

    private _syncWidgets(): void {
    ...
    /**
     * @param dtMs - Frame duration in milliseconds.
     * @param engine - Active Babylon engine.
     */
    update(dtMs: number, engine: AbstractEngine): void {
    ...
    private _copyPose(btn: HTMLButtonElement): void {
    ...
    private _txt(el: TextElement, s: string): void {
        if (el._v !== s) {
            el._v = s;
            el.textContent = s;
        }
...
function signDeg(d: number): string {
...
function setDynamic(key: SettingKey, value: SettingValue): void {
    set(key, value);
}
