/**
 * The snow-surf wake Р Р†Р вЂљРІР‚Сњ the centrepiece.
 *
 ...
 * moment the player turns, and the failure is not subtle Р Р†Р вЂљРІР‚Сњ the spray comes off
...
import { ExaltedWorld } from "../terrain/exaltedWorld.ts";
 ...
 * `LIFE * speed` Р Р†Р вЂљРІР‚Сњ 16 m at the controller's top speed, four metres at a jog.
 ...
 *
 * Taller than the character, deliberately. A physically plausible 1.45 m Р Р†Р вЂљРІР‚Сњ what
 * a real snowboard carve throws Р Р†Р вЂљРІР‚Сњ reads as a ripple at this framing, nine metres
...
export class SurfWake {
    ...
    private readonly _x: Float32Array;
    private readonly _y: Float32Array;
    private readonly _z: Float32Array;
    private readonly _rx: Float32Array;
    private readonly _rz: Float32Array;
    private readonly _travel: Float32Array;
    private readonly _laid: Float32Array;
    private readonly _strength: Float32Array;
    private readonly _carve: Float32Array;
    private _head: number;
    private _count: number;
    private _odo: number;
    private _clock: number;
    private _active: boolean;
    private _plumeOwed: number;
    private _driftOwed: number;
    private readonly _ampL: Float32Array;
    private readonly _ampR: Float32Array;
    private readonly _dist: Float32Array;
    private readonly _col: Int32Array;
    ...
    private readonly _camPos: Vector3;
    private _enabled: boolean;
    debug: number;

    constructor(
        scene: Scene, sky: Sky, shadows: ShadowSystem,
        controller: CharacterController, spray: SprayField, terrain: Terrain,
    ) {
        this.scene = scene;
        this.sky = sky;
    ...
    private _makeMaterial(): ShaderMaterial {
        ...
        const exalted = world instanceof ExaltedWorld ? world : null;
        mat.setTexture('surfaceMap', exalted?.surfaceMap || this.sky.lut);
        mat.setVector2('surfaceOrigin', exalted?.origin || Vector2.Zero());
        mat.setVector2('surfaceExtent', exalted?.extent || Vector2.Zero());
    ...
    private _makeDepthMaterial(cascade: number): ShaderMaterial {
        const mat = new ShaderMaterial(
     ...
     * where the player stopped and where they started again Р Р†Р вЂљРІР‚Сњ which, if they
     ...
     * geometry Р Р†Р вЂљРІР‚Сњ so it is decided once, here, and shipped to the shader as a
     * pair of resolved amplitudes rather than as a carve the vertex shader would
     * have to re-interpret.
     *
     * @returns The largest amplitude anywhere on the wave, in metres.
     */
     ...
     * Spray off the lip.
     *
     * Emitted from the crest position the mesh is drawing Р Р†Р вЂљРІР‚Сњ same spine, same
     * amplitude, same side weighting Р Р†Р вЂљРІР‚Сњ so the plume leaves the wave rather than
     * appearing near it. Rate is per metre travelled, not per second, so it does
     * not thin out at a higher frame rate.
     */
    private _plume(dt: number): void {
        const ch = this.controller;
        const sp = this.spray;
        const n = this._count;
        ...
        if (count > 0) {
            ...
            // Sample the live part of the wave Р Р†Р вЂљРІР‚Сњ roughly the first four metres,
            ...
            for (let k = 0; k < count; k++) {
                ...
                // inside that Р Р†Р вЂљРІР‚Сњ so the shed grains leave from a band straddling
                ...
                // crest Р Р†Р вЂљРІР‚Сњ which is where the wall is actually coming apart Р Р†Р вЂљРІР‚Сњ while
                ...
                sp.emit(
                    ...
                    // has to actually clear the wave Р Р†Р вЂљРІР‚Сњ see `drag` in particles.ts.
 ...
 * at all Р Р†Р вЂљРІР‚Сњ see `wake.vertex.wgsl`.
 */
function buildLattice(scene: Scene): Mesh {
