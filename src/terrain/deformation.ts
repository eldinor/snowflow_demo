...
interface DeformationFocus { x: number; z: number }
interface DeformationSettings { deformResolution:number; refillRate:number; deformDepth:number; deformBerm:number; windDirection:number }
const DS = S as unknown as DeformationSettings;
...
export class DeformationField {
    ...
    private readonly _prevCenter: Vector2;
    private readonly _brushData: Float32Array;
    _brushCount = 0;
    readonly brushTex: RawTexture;
    private readonly _targets: [ProceduralTexture, ProceduralTexture];
    private _write = 0;
    private _relaxOwed = 0;
    texture: ProceduralTexture;
    private _warmed = false;
    private _brushDirty = false;
    surfaceMap?: BaseTexture;
    surfaceWeightsAt?: (x:number,z:number)=>ArrayLike<number>;

    constructor(scene: Scene) {
        this.scene = scene;
        this.res = Math.max(512, DS.deformResolution | 0);
        this.size = COVERAGE;
    ...
    /** @param {number} i */
    private _makeTarget(i: number): ProceduralTexture {
     ...
     */
    brush(x:number,z:number,radius:number,depth:number,berm:number,compression:number,ice:number,yaw=0,elongation=1,edge=1): void {
     ...
     */
    update(dt:number, focus:DeformationFocus): ProceduralTexture {
        ...
        pt.setFloat("refillRate", DS.refillRate);
        pt.setFloat("maxDepth", 0.55 * DS.deformDepth);
        pt.setFloat("maxBerm", 0.34 * DS.deformBerm);
        pt.setFloat("windAngle", (DS.windDirection * Math.PI) / 180);
     ...
     */
    setSurfaceMap(texture:BaseTexture, origin:Vector2, extent:Vector2): void {
