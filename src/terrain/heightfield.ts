...
import { bakeOnce } from "../core/gpuUtil.ts";
...
export class Heightfield {
    ...
    readonly heightTex: ProceduralTexture;
    readonly auxTex: ProceduralTexture;
    ...
    async bake(): Promise<void> {
        const windDirection = numericSetting('windDirection', S.windDirection);
        const macroHeightScale = numericSetting('macroHeightScale', S.macroHeightScale);
        ...
        this.heightTex.setFloat("heightAmp", macroHeightScale);
    ...
    private async _readback(): Promise<void> {
        ...
        if (!(raw instanceof Float32Array)) {
            throw new Error(`Heightfield expected Float32Array readback, received ${raw.constructor.name}.`);
        }
        const src = raw;
    ...
    clampToPlayArea(v: { x: number; z: number }): void {
        const r = PLAY_RADIUS;
...
function bsplineWeights(t: number, out: Float32Array): void {
    const t2 = t * t;
    const t3 = t2 * t;
...
function numericSetting(name: string, value: string | number | boolean): number {
    if (typeof value !== 'number') throw new TypeError(`${name} setting must be numeric.`);
    return value;
}
