...
export const systemMs: Record<string, number> = Object.create(null) as Record<string, number>;
 ...
 * @param {number} ms
 */
export function sample(ms: number): void {
    times[head] = ms;
    head = (head + 1) % CAP;
    if (count < CAP) count++;
    stats.last = ms;
 ...
 */
export function installDrawCounter(engine: AbstractEngine): void {
    const elements = engine.drawElementsType.bind(engine);
    const arrays = engine.drawArraysType.bind(engine);
    engine.drawElementsType = function (a: number, b: number, c: number, d?: number): void {
        drawAccum++;
        elements(a, b, c, d);
    };
    engine.drawArraysType = function (a: number, b: number, c: number, d?: number): void {
        drawAccum++;
        arrays(a, b, c, d);
    };
}
...
export function endFrameDraws(): void {
 ...
 */
export function checkSpike(ms: number): void {
    spikes.sinceReset++;
 ...
 */
export function resetSpikes(): void {
    spikes.count = 0;
    spikes.sinceReset = 0;
}
