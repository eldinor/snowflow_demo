 ...
 */
export const SCHEMA: SettingGroup[] = [
    {
        group: "Sun & Sky",
        items: [
...
type BroadListener = (value: SettingValue, key: SettingKey) => void;
export type SettingListener<K extends SettingKey> = (value: Settings[K], key: K) => void;

const listeners = new Map<SettingKey, Set<BroadListener>>();

/** Subscribe to one or more settings keys with values inferred from those keys; returns an unsubscribe function. */
export function onChange<K extends SettingKey>(
    keys: K | readonly K[],
    fn: SettingListener<K>,
): () => void {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        set.add(fn as unknown as BroadListener);
    }
    return () => {
        for (let i = 0; i < list.length; i++) listeners.get(list[i])?.delete(fn as unknown as BroadListener);
    };
}
 ...
 */
export function set<K extends SettingKey>(k: K, v: Settings[K]): void {
    if (S[k] === v) return;
    S[k] = v;
...
export function applyPreset(name: PresetName): void {
    ...
    if (!p) return;
    S.preset = name;
    for (const [key, value] of Object.entries(p) as Array<[SettingKey, SettingValue]>) {
        set(key, value);
    }
}
