...
interface SkySettings {
    sunAzimuth:number; sunElevation:number; sunIntensity:number; sunTempWarm:number;
    windDirection:number; ambientIntensity:number; showMountains:boolean; mountainHeight:number;
    fogDensity:number; fogHeightFalloff:number; fogStart:number; aerialStrength:number;
}
const SS = S as unknown as SkySettings;
interface SkyCameraRig { camera:{position:Vector3;maxZ:number} }

/** Bake atmospheric lookup data and ambient lighting shared by world materials. */
export class Sky {
    readonly scene: Scene;
    readonly engine: AbstractEngine;
    readonly sunDir = new Vector3(0, .2, 1);
    readonly sunColor = new Color3(1, .85, .66);
    readonly sunRadiance = new Color3(1, 1, 1);
    sunScale = 1;
    readonly groundBounce = new Color3(0, 0, 0);
    readonly sh = new Float32Array(36);
    private _dirty = true;
    readonly lut: ProceduralTexture;
    readonly shLut: ProceduralTexture;
    readonly mesh: Mesh;
    ...
    syncFromSettings(): void {
        const az = (SS.sunAzimuth * Math.PI) / 180;
        const el = (SS.sunElevation * Math.PI) / 180;
        ...
        this.sunScale = SS.sunIntensity * SUN_SCALE_BASE;
    ...
    private _updateGroundBounce(): void {
    ...
    private _irradianceUp(): Float32Array {
        const sh = this.sh;
        const out = _irrTmp;
        for (let k = 0; k < 3; k++) {
     ...
     */
    async projectSH(): Promise<void> {
        const data = await this.shLut.readPixels(0, 0);
        if (!data) return;
        if (!(data instanceof Float32Array)) throw new Error(`Sky SH expected Float32Array, received ${data.constructor.name}.`);
        const px = data;
    ...
    /** @param {import("../core/camera.ts").CameraRig} rig */
    render(rig:SkyCameraRig, time:number): void {
        const a = (SS.windDirection * Math.PI) / 180;
        ...
        m.setArray4("shR", this.sh as unknown as number[]);
        m.setFloat("ambientIntensity", SS.ambientIntensity);
        m.setFloat("ridgeAmp", SS.showMountains ? SS.mountainHeight : 0);
        ...
        m.setFloat("fogDensity", SS.fogDensity);
        m.setFloat("fogHeightFalloff", SS.fogHeightFalloff);
        m.setFloat("fogStart", SS.fogStart);
        m.setFloat("aerialStrength", SS.aerialStrength);
    }
