...
const NDC: ReadonlyArray<readonly [number,number,number]> = [
...
export class ShadowSystem {
    ...
    readonly maps: RenderTargetTexture[] = [];
    readonly materials: ShaderMaterial[] = [];
    readonly matrices: Matrix[] = [];
    readonly matrixData = new Float32Array(16 * CASCADE_COUNT);
    readonly splits = new Float32Array(4);
    readonly texelSize = 1 / RESOLUTION;
    readonly resolution = RESOLUTION;
    readonly params: Vector4[] = [];
    readonly paramData = new Float32Array(4 * CASCADE_COUNT);
    readonly lightDir = new Vector3(0, -1, 0);
    minHeight = -60;
    maxHeight = 60;
    texelWorldPad = 2;
    private _perCascade?: ShaderMaterial[][];
     ...
     */
    setHeightBounds(min:number, max:number): void {
        this.minHeight = min;
        this.maxHeight = max;
    }
     ...
     */
    registerCaster(mesh:Mesh, makeMaterial:(cascade:number)=>ShaderMaterial, cascades?:number): void {
     ...
     */
    update(camera:Camera, sunDir:Vector3): void {
     ...
     */
    private _fitCascade(c:number, sliceNear:number, sliceFar:number, camNear:number, camFar:number): void {
