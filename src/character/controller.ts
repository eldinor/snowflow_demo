...
import type { MovementTerrain } from './movementTypes.ts';
...
interface ControllerTerrain extends MovementTerrain {
    normalAt<T extends Vector3>(x: number, z: number, out: T): T;
}

interface MovementCameraRig {
    yaw: number;
    getFlatForward(out: Vector3): Vector3;
    getFlatRight(out: Vector3): Vector3;
    addTrauma(amount: number): void;
}

/** Own movement and mode transitions; expose derived state for the figure, cloth and effects. */
export class CharacterController {
    terrain: ControllerTerrain;
    flight: Flight;
    swimming: Swimming;
    position: Vector3;
    velocity: Vector3;
    ...
    constructor(terrain: ControllerTerrain) {
     ...
     */
    update(dt: number, rig: MovementCameraRig): void {
    ...
    private _walkStep(h: number): void {
    ...
    private _surfStep(h: number, rig: MovementCameraRig): void {
...
export function angleDamp(cur: number, target: number, rate: number, dt: number): number {
