 ...
 *
 *   body   linearly blend-skinned to the bones РІР‚вЂќ head, cowl, torso, arms,
 *          trousers, boots, belt.
 ...
 *
 * Build time only РІР‚вЂќ none of this runs after load, and it allocates freely.
 *
...
import type { ClothPanel } from './cloth.ts';
import {
...
type Vec3Output = number[] | Float32Array;
type BoneBlend = [number, number, number, number];
type LoftRing = [number, number, number, number, number, number, number, number, number, number];
...
class Builder {
    ...
    constructor() {
        // Arrays remain growable during the one-time authoring pass and are
        // packed into typed GPU buffers by `finishSkinned`.
    }

    /** @returns {number} the new vertex's index */
    vert(x: number, y: number, z: number, u: number, v: number, matId: number,
        ao: number, b0: number, w0: number, b1: number, w1: number): number {
        this.pos.push(x, y, z);
        this.nrm.push(0, 0, 0);
        this.uv.push(u, v);
    ...
    normal(vi: number, x: number, y: number, z: number): void {
        this.nrm[vi * 3] = x;
        this.nrm[vi * 3 + 1] = y;
        this.nrm[vi * 3 + 2] = z;
    }
 ...
 */
function loft(B: Builder, rings: LoftRing[], matId: number, ref: Vec3,
    capStart: boolean, capEnd: boolean): void {
    ...
    for (let r = 0; r < n; r++) {
        ...
        // U = axis x ref, W = axis x U РІР‚вЂќ the two axes of the section plane.
        ...
        // scale in the fabric shader РІР‚вЂќ the weave, the yarn slub РІР‚вЂќ is a physical
    ...
    if (capEnd && prevRow) capRing(B, rings[n - 1], rings[n - 2], prevRow, matId, false);
}

function capRing(B: Builder, ring: LoftRing, neighbour: LoftRing, row: number[],
    matId: number, isStart: boolean): void {
...
function spineBones(y: number): BoneBlend {
    if (y < 1.06) {
...
function ring(cx: number, cy: number, cz: number, rx: number, rz: number,
    ao: number, bones: BoneBlend): LoftRing {
...
function limbRings(x0: number, y0: number, z0: number, x1: number, y1: number,
    z1: number, r0: number, r1: number, steps: number, boneA: number,
    boneB: number, ao: number, from: number, to: number): LoftRing[] {
    const out: LoftRing[] = [];
 ...
 *
 * Most of this is only seen in slivers РІР‚вЂќ the robe covers the torso, the mantle
 ...
 */
export function buildBody(scene: Scene): Mesh {
    const B = new Builder();

    // ---- torso ------------------------------------------------------------
    const torso: LoftRing[] = [];
    const TORSO = [
...
const HOOD_COLS = 34;
const HOOD_ROWS = 9;
const HEAD_C: Vec3 = [0, 1.655, 0.005];
const FACE_DIR = (() => {
    const v: Vec3 = [0, -0.28, 0.96];
    const l = Math.hypot(v[0], v[1], v[2]);
    return [v[0] / l, v[1] / l, v[2] / l];
})();

/** Face-opening rim point at parameter `s` (0 = crown, 0.5 = under the chin). */
export function hoodRimPoint<T extends Vec3Output>(s: number, out: T): T {
    const a = s * Math.PI * 2;
...
function hoodBasePoint<T extends Vec3Output>(s: number, out: T): T {
    const a = s * Math.PI * 2;
...
function buildHood(B: Builder): void {
    const rim: Vec3 = [0, 0, 0];
    ...
    for (let r = 0; r <= HOOD_ROWS; r++) {
        ...
        for (let c = 0; c < HOOD_COLS; c++) {
            ...
            // to down-and-forward under the chin РІР‚вЂќ which is the same sweep the
 ...
 * strand field whose threshold rises with the shell parameter РІР‚вЂќ so strands
 ...
 * solve РІР‚вЂќ a second vertex program, for very little visible gain.
 */
export function buildFur(scene: Scene): Mesh {
    const B = new Builder();
 ...
 * @param {number} ao
 */
function emitFurBand(B: Builder, cols: number, bases: Float32Array, outs: Float32Array,
    r0: number, len: number, shells: number, bone: number, ao: number): void {
    ...
    for (let s = 0; s < shells; s++) {
        ...
        // to its neighbours. That is the whole idea РІР‚вЂќ the gaps between them are
...
function finishSkinned(scene: Scene, name: string, B: Builder, isFur = false): Mesh {
 ...
 *
 * It carries no positions of its own РІР‚вЂќ `position` is `(u, v, panelIndex)` and
 ...
 *
 * @param {import("./cloth.ts").ClothPanel[]} panels
 */
export function buildClothMesh(scene: Scene, panels: ClothPanel[]): Mesh {
    const pos: number[] = [];
    const uv: number[] = [];
    const aux: number[] = [];
    const idx: number[] = [];
