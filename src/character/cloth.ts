/**
 * Garment simulation вЂ” verlet cloth on coarse grids.
 *
 ...
 * each frame is that bind position pushed through the bone's skinning matrix вЂ”
 ...
 *   1-5        follows loosely вЂ” this is where a garment starts to read as cloth.
 ...
 *
 * Wind is *apparent* wind вЂ” the field wind minus the character's own velocity вЂ”
...
interface HeightSurface {
    heightAt(x: number, z: number): number;
}

type CurvePoint = [number, number, number];
...
export class ClothPanel {
    ...
    readonly weaveU: number;
    readonly weaveV: number;
    readonly aoTop: number;
    readonly aoBottom: number;
    readonly collide: number;
    readonly groundRows: number;
    nodeRow = 0;
    readonly count: number;
    readonly bindPos: Float32Array;
    readonly pos: Float32Array;
    readonly prev: Float32Array;
    readonly target: Float32Array;
    readonly bone: Int32Array;
    readonly pinRate: Float32Array;
    readonly restU: Float32Array;
    readonly restV: Float32Array;
    readonly restB: Float32Array;

    constructor(spec: ClothPanelSpec) {
        this.name = spec.name;
        this.cols = spec.cols;
        this.rows = spec.rows;
...
function dist3(a: Float32Array, ia: number, b: Float32Array, ib: number): number {
...
function curve(table: CurvePoint[], t: number): [number, number] {
    let i = 0;
...
function makeRobe(): ClothPanel {
    ...
    for (let j = 0; j < p.rows; j++) {
        ...
        for (let i = 0; i < p.cols; i++) {
            ...
            // The flare accelerates downward, and the back flares furthest вЂ”
            ...
            // the crest of a fold, where there is most fabric to hang вЂ” in
 ...
 * from fifteen metres.
 */
function makeMantle(): ClothPanel {
    const p = new ClothPanel({
 ...
 */
function makeSleeve(side: 0 | 1): ClothPanel {
    const s = side === 0 ? -1 : 1;
    const p = new ClothPanel({
 ...
 */
export function makePanels(): ClothPanel[] {
...
const CAPSULES: Array<[number, number, number, number]> = [
...
export class ClothSolver {
    readonly panels: ClothPanel[];
    readonly terrain: HeightSurface;
    private readonly _wind: Float32Array;
    private readonly _acc: Float32Array;
    private _t: number;

    constructor(panels: ClothPanel[], terrain: HeightSurface) {
        this.panels = panels;
    ...
    update(dt: number, fig: Figure, ch: CharacterController): void {
        ...
        // ---- apparent wind ----------------------------------------------
        const a = (Number(S.windDirection) * Math.PI) / 180;
        const ws = 3.2 * Number(S.windStrength);
    ...
    private _step(p: ClothPanel, h: number, fig: Figure): void {
        const n = p.count;
        const pos = p.pos;
        const prev = p.prev;
    ...
    private _anchors(p: ClothPanel, h: number): void {
        const n = p.count;
        const pos = p.pos;
        const target = p.target;
        for (let k = 0; k < n; k++) {
     ...
     */
    private _distance(p: ClothPanel, iteration: number): void {
    ...
    private _collide(p: ClothPanel, fig: Figure): void {
        const n = p.count;
        const pos = p.pos;
...
/**
 * One distance constraint. Mass weighting is binary вЂ” a welded particle does not
 * move at all вЂ” which is both correct and much cheaper than carrying inverse
