 ...
 * the only thing standing between the physics in `controller.ts` and the marks
...
interface ContactFigure { touchdown: ArrayLike<boolean>; plant: ArrayLike<number> }
interface ContactSpray {
    emit(x:number,y:number,z:number,vx:number,vy:number,vz:number,size:number,life:number,kind:number): void;
}
type ContactCharacter = CharacterController & {
    terrain: CharacterController['terrain'] & {
        heightfield?: { weightsAt?(x:number,z:number): ArrayLike<number> };
    };
};

/**
 * Boot geometry, metres. `WIDTH` is the short-axis radius, so the print is
 * 20 cm across and 34 cm long РІР‚вЂќ a boot plus the collapse of the snow around it,
...
export class SnowContact {
    /**
     * @param {import("./controller.ts").CharacterController} character
     * @param {import("../terrain/deformation.ts").DeformationField} field
     * @param {import("./figure.ts").Figure} [figure] posed skeleton, if built
     * @param {import("../vfx/particles.ts").SprayField} [spray]
     */
    readonly character: ContactCharacter;
    readonly field: DeformationField;
    readonly spray: ContactSpray | null;
    readonly figure: ContactFigure | null;
    private _sinceSplat = 0;
    private _prevX: number;
    private _prevZ: number;

    constructor(character: ContactCharacter, field: DeformationField, figure?: ContactFigure, spray?: ContactSpray) {
    ...
    update(dt: number): void {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            f.brush(
                ...
                ch.facing,
                BOOT_ELONG,
                1.0                     // full rim roughness РІР‚вЂќ boots tear edges
            );
     ...
     * ground on the exact frame the foot arrives РІР‚вЂќ one event, rather than two
     ...
     */
    private _kick(x:number, y:number, z:number, impact:number): void {
    ...
    /**
     * Walking scuff. Very shallow, and only while actually moving РІР‚вЂќ a standing
     * character should not slowly bore a hole.
     */
    private _walk(_dt:number, moved:number): void {
        const ch = this.character;
        ...
        // ends up at is roughly rate * 2 * radius * profile РІР‚вЂќ independent of
        ...
        // footprints into one continuous ski track РІР‚вЂќ fine while the feet were
     ...
     */
    private _surf(_dt:number, moved:number): void {
        const ch = this.character;
        const f = this.field;
        const s = ch.surf;
        ...
        // means extra snow moved РІР‚вЂќ it goes into width and into the walls, which
        ...
        f.brush(
            ...
            1.20 * k,   // deep РІР‚вЂќ a run should be visible from across the field
            0.30 * k,
        ...
        // turn is the left-hand side РІР‚вЂќ the board resists the turn and throws snow
        ...
        //
        // The wake mesh in `src/vfx/surfWake.ts` resolves its sides from the same
