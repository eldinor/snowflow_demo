...
export class Bloom {
    ...
    /**
     * @param x - Ground target X.
     * @param y - Ground target Y.
     * @param z - Ground target Z.
     */
     ...
     */
    private _curtain(dt: number): void {
        const ctx = this.ctx;
        const sp = ctx.spray;
        if (!sp) return;
