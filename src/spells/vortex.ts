...
import { PROFILE_TUBE } from "./waterBody.ts";
import { clamp01, smooth01, bell, transport } from "./bending.ts";
...
export class Vortex {
    ...
    active = false;
    readonly strands: number[] = [-1, -1, -1];
    t = 0;
    x = 0;
    z = 0;
    spin = 0;
    private _stripOwed = 0;
    private _grainOwed = 0;
    ...
    private _helices(env: number): void {
        const ctx = this.ctx;
     ...
     */
    private _strip(dt: number, env: number): void {
        const ctx = this.ctx;
        const f = ctx.deform;
     ...
     */
    private _grains(dt: number, env: number): void {
        const ctx = this.ctx;
        const sp = ctx.spray;
