...
import { smooth01 } from "./bending.ts";
...
export class Aegis {
    ...
    rz = 0;
    private _seed = 0;
    private readonly _handles = new Int16Array(MAX);
    private readonly _x = new Float32Array(MAX);
    private readonly _y = new Float32Array(MAX);
    private readonly _z = new Float32Array(MAX);
    private readonly _height = new Float32Array(MAX);
    private _frostOwed = 0;
    ...
    trigger(x: number, z: number, fx: number, fz: number): void {
        if (this.hasWall) {
            this.shatter();
            return;
        }
    ...
    private _frost(dt: number, strength: number): void {
