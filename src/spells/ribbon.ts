...
import { PROFILE_TUBE, STRAND_COLS } from "./waterBody.ts";
import { clamp01, clampRange, smooth01, expDamp, transport } from "./bending.ts";
import type { SpellContext } from "./spellSystem.ts";
...
export class Ribbon {
    ...
    held = false;
    strand = -1;
    private readonly _x = new Float32Array(SAMPLES);
    private readonly _y = new Float32Array(SAMPLES);
    private readonly _z = new Float32Array(SAMPLES);
    private readonly _spd = new Float32Array(SAMPLES);
    private _head = 0;
    private _count = 0;
    tipX = 0;
    tipY = 0;
    ...
    blend = 0;
    private _sprayOwed = 0;
    private _scoreOwed = 0;
    private _retireOwed = 0;
     ...
     */
    private _driveTip(dt: number): void {
        const ctx = this.ctx;
        const rig = ctx.rig;
    ...
    private _push(x: number, y: number, z: number): void {
     ...
     * the force on it.
     */
    private _retire(dt: number): void {
