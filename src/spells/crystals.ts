...
export class CrystalField {
    ...
    prepassMat: ShaderMaterial | null = null;
    ...
    private _makeMaterial(): ShaderMaterial {
        ...
        // degenerate and a growing one is very thin Р Р†Р вЂљРІР‚Сњ culling buys nothing here
    ...
    plant(
        ...
        height: number, radius: number, growSeconds: number, life: number,
    ): number {
        let i = this._next;
    ...
    meltNear(x: number, z: number, radius: number, amount: number): number {
        const d = this._texData;
     ...
     * module, but the WebGPU render pipeline Р Р†Р вЂљРІР‚Сњ blend state, depth state, target
     * formats Р Р†Р вЂљРІР‚Сњ is only built when a triangle actually goes through it. Hiding
...
function buildMesh(scene: Scene): Mesh {
