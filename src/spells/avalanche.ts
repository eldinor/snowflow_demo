...
import { PROFILE_SHEET } from "./waterBody.ts";
import { clamp01, smooth01, bell } from "./bending.ts";
...
export class Avalanche {
    ...
    /**
     * @param ax - Horizontal aim X.
     * @param az - Horizontal aim Z.
     */
    trigger(ax: number, az: number): void {
    ...
    private _track(travelled: number, env: number, halfWidth: number): void {
        if (env < 0.06) return;
    ...
    private _spray(travelled: number, env: number, halfWidth: number, height: number): void {
