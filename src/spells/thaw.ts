...
import { clamp01, smooth01 } from "./bending.ts";
...
export class Thaw {
    ...
    y = 0;
    z = 0;
    private _pulseOwed = 0;
    private _mistOwed = 0;
    private _count = 0;
    private readonly _px = new Float32Array(STRAND_COLS);
    private readonly _py = new Float32Array(STRAND_COLS);
    private readonly _pz = new Float32Array(STRAND_COLS);
    private readonly _dist = new Float32Array(STRAND_COLS);
    ...
    trigger(x: number, y: number, z: number, aimX: number, aimZ: number): void {
    ...
    private _buildRivulet(aimX: number, aimZ: number): void {
    ...
    private _rivulet(grow: number, fade: number): void {
    ...
    private _mist(dt: number, radius: number, env: number): void {
