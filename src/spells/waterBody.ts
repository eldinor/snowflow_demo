/**
 * The bent-water renderer РІР‚вЂќ one mesh, one material, one draw, eight strands.
 *
 ...
 * vertical, and a Frenet or up-referenced frame flips there РІР‚вЂќ the section spins
...
import { SPELL_LIGHT_UNIFORMS } from "./spellLights.ts";
import type { ShadowSystem } from '../render/shadows.ts';
import type { Sky } from '../render/sky.ts';
import type { SpellLights } from './spellLights.ts';

/** Must match `array<vec4f, 8>` in `water.vertex.wgsl`. */
export const STRAND_MAX = 8;

/**
 * Spine *samples* per strand РІР‚вЂќ the width of the data texture, and the most
 ...
 * that is zero at every knot and peaks in the middle of each span РІР‚вЂќ which is
...
/**
 * Spine *vertices* per strand РІР‚вЂќ the width of the lattice.
 *
 ...
 * the body comes out visibly segmented РІР‚вЂќ the thing that makes a swept tube look
 ...
 *
 * The last one coincides with the first РІР‚вЂќ a tube is closed, so ring 17 sits at
 ...
 * detail the relief field is allowed to put *around* the section РІР‚вЂќ and detail
...
export type WaterProfile = typeof PROFILE_TUBE | typeof PROFILE_SHEET;
...
export class WaterBody {
    ...
    constructor(scene: Scene, sky: Sky, shadows: ShadowSystem, lights: SpellLights) {
        this.scene = scene;
        this.sky = sky;
    ...
    private _makeMaterial(): ShaderMaterial {
        ...
        // A transparent body seen from both sides РІР‚вЂќ looking through the near wall
     ...
     */
    setParams(s: number, profile: WaterProfile, milkiness: number, alpha: number, count: number): void {
        const p = s * 4;
     ...
     * have to be *transported* РІР‚вЂќ see the note at the top of the file.
     *
     *
    ...
    column(
        ...
        rx: number, ry: number, rz: number, twist: number,
        dist: number, age: number, foam: number, flatten: number,
    ): void {
    ...
    private _pushUniforms(): void {
        ...
        m.setArray4("strandParams", this._params as unknown as number[]);
     ...
     * frame of the first cast instead РІР‚вЂќ a ~250 ms freeze behind a warm-up that
     * looks complete.
     */
