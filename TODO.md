...
- Use Snowflow as the avatar and snow-biome visual reference.
...
- [x] Read the Snowflow avatar/rendering code and Exalted handoff and map reference.
...
- [x] Place the Snowflow avatar at the authored desert spawn, facing north.
...
- [x] Apply Snowflow snow shading to the near-white snow biome and disable the procedural background mountains in Exalted mode.
- [x] Keep the original procedural demo accessible at `?terrain=snowflow`.
...
- [x] Reuse Snowflow footprint, sliding and spell brushes; sand receives 55% depth, 65% berm height and three-times-faster refill.
- [x] Add warm sand particles and wake shading while retaining Snowflow's snow appearance.
...
- Exalted's documented movement values differ from Snowflow's current controller: walk 7 m/s, Earth surge 22 m/s and Water flow 24 m/s. These have not been adopted automatically.
...
- [x] Integrate Snowflow's geometric deformation into Exalted's snow and loose desert sand; preserve the original `?terrain=snowflow` reference mode.
...
- Ground queries are not a general obstacle, swimming or slope-limit physics system. Imported props will need separate interaction/collision decisions.
